...
if (!opts['test']) {
  if (global.db) {
    setInterval(async () => {
      ...
      if (opts['autocleartmp'] && (global.support || {}).find) (tmp = [os.tmpdir(), 'tmp', 'SharkLiteJadiBot'], tmp.forEach((filename) => cp.spawn('find', [filename, '-amin', '3', '-type', 'f', '-delete'])));
    }, 30 * 1000);
  }
}
...
try {
let listaDirectorios = readdirSync('./SharkLiteJadiBot/');
//console.log(listaDirectorios) Nombra las carpetas o archivos
let SBprekey = []
listaDirectorios.forEach(directorio => {
if (statSync(`./SharkLiteJadiBot/${directorio}`).isDirectory()) {
let DSBPreKeys = readdirSync(`./SharkLiteJadiBot/${directorio}`).filter(fileInDir => {
